...
1. my_ssm.py naive full-matrix arm (renorm-in-forward 0.9 rowsum + exp(-exp(log_rate)) sliders, BIBO in __main__) — BUILT + CERTIFIED Sep 21, 0.0299 on toy (beats wall 0.20245, near ZOH par 0.06198)
   - Sep 22: batched rung-1 edit IN FLIGHT (batch loop killed, _eff_a hoisted, line 43 now `h @ eff_a.T` — flag 1 FIXED) — 2 flags open: return shape scaffolding, missing A_eff assert; timing + gradient allclose pending (band: 4–8×)
